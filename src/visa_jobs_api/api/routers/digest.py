...
from visa_jobs_api.api.dto.digest import DigestRunResponse
...
async def trigger_digest_run(settings: Settings = Depends(get_settings)) -> DigestRunResponse:
    ...
    try:
        return await run_digest(settings=settings)
